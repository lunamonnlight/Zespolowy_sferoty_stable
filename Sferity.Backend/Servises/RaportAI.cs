...
public class RaportAI
{
    ...
    public async Task<AIRaport> GenerateRaport(object jsonData, long krsReportId, string type)
    {
        ...
        {
            case "Finance":
                promptFile = await File.ReadAllTextAsync("Files/promt-finance.txt");
                break;

            case "Prawnik":
                promptFile = await File.ReadAllTextAsync("Files/promt-prawnik.txt");
                break;

            case "Sales":
                promptFile = await File.ReadAllTextAsync("Files/promt-sales.txt");
                break;

            default:
                promptFile = await File.ReadAllTextAsync("Files/promt.txt");
                break;
        }
        
        //var promptFile = await File.ReadAllTextAsync("../Files/promt.txt");
        var json = JsonSerializer.Serialize(jsonData, new JsonSerializerOptions { WriteIndented = true });
        var prompt = promptFile.Replace("{jsonData}", json);
