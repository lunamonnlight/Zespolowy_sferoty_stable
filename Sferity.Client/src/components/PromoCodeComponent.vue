...
// Formularz generowania (Tylko niezbędne pola)
...
const generateCodes = async () => {
  ...
  try {
    ...
    if (response.ok) {
      toast.add({ severity: 'success', summary: 'Sukces', detail: `Wygenerowano kody GUID`, life: 3000 })
      genQuantity.value = 1
      await fetchCodes()
    }
  } catch (error) {
    toast.add({ severity: 'error', summary: 'Błąd', detail: 'Błąd połączenia', life: 3000 })
  } finally {
...
const copyToClipboard = (text) => {
  ...
  toast.add({ severity: 'secondary', summary: 'Skopiowano', detail: 'GUID został skopiowany', life: 2000 })
}
  ...
  <div class="space-y-12 font-sans text-surface-0">
    ...
    <div class="flex justify-between items-end border-b border-surface-600 pb-6 mt-4">
      <div>
        ...
        <p class="text-sm text-surface-400 tracking-wide mt-1">Zarządzanie partiami GUID</p>
      </div>
    ...
    <!-- Kreator -->
    <div class="bg-surface-700/30 p-6 rounded-lg border border-surface-600 space-y-6">
    ...
    <div class="bg-surface-700/30 p-6 rounded-lg border border-surface-600 space-y-6 mt-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 pb-2">
        <div class="flex flex-col gap-2">
          <label class="text-[10px] text-surface-400 uppercase tracking-widest">Wyszukaj GUID</label>
          <InputText v-model="searchQuery" placeholder="Wklej fragment kodu..." class="w-full p-inputtext-sm bg-surface-900" />
        </div>
        <div class="flex flex-col gap-2">
      ...
      <DataTable
          :value="processedCodes" dataKey="code"
          paginator :rows="10"
          size="small"
      ...
      >
        <Column field="code" header="Identyfikator (GUID)">
          <template #body="{ data }">
            <div class="flex items-center gap-2 group">
              <span class="text-[11px] font-mono text-primary-400">{{ data.code }}</span>
        ...
        <Column field="status" header="Status">
          <template #body="{ data }">
            <Tag :severity="getStatusDetails(data.status).severity" :value="getStatusDetails(data.status).label" variant="outlined" class="text-[10px] uppercase" />
          </template>
        </Column>
